...
function getI18n({ defaultLanguage = 'en', source, view }) {
  ...
  lines.forEach((line, index) => {
    ...
    if (isText(line)) {
      ...
      const isTranslation = regexList.some(regex =>
        regex.test(lines[index - 1])
      )
      ...
      if (isTranslation && textKey) {
        ...
        obj[langShortCode][`${view}/${currentBlock}/${textKey}`] = withoutSlot(
          text
        )
